...
export async function GET(request: Request) {
  ...
  try {
    ...
    const detail = await neracaService.getLabaRugiDetail(session.user.company!.id, filter);
    return NextResponse.json(detail);
  } catch (error) {
    console.error("Error fetching laba rugi detail:", error);
    return NextResponse.json(
      { error: "Failed to fetch laba rugi detail" },
      { status: 500 }
    );
  }
}

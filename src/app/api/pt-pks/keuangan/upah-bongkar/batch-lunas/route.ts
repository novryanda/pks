...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await keuanganDashboardService.markUnloadingWagesPaid(
      companyId,
      "PENERIMAAN_TBS",
      data.penerimaanIds,
      userName,
    ...
    console.error("Error batch settling unloading wage:", error);
    return NextResponse.json(
      { error: error.message || "Failed to settle unloading wage" },
      { status: 400 },
    );
  }
}

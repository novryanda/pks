...
export async function GET(request: Request) {
  ...
  try {
    ...
    const pengirimans = await pengirimanProductService.getPendingMutu(companyId);
    return NextResponse.json(pengirimans);
  } catch (error: any) {
    console.error("Error fetching pending mutu:", error);
    return NextResponse.json(
      { error: error.message || "Failed to fetch pending mutu" },
      { status: 500 }
    );
  }
}

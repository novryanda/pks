...
type UpahBongkarResponse = {
  items: UpahBongkarItem[];
  summary: {
    totalNilai: number;
    totalDibayar: number;
    sisaHutang: number;
...
const getStatusBadge = (status: UpahBongkarItem["status"]) => {
  if (status === "PAID") {
...
export function UpahBongkarList() {
  const [response, setResponse] = useState<UpahBongkarResponse | null>(null);
  ...
  const [selectedItem, setSelectedItem] = useState<UpahBongkarItem | null>(null);
  ...
  const fetchData = async (showRefreshState = false) => {
    try {
      ...
      if (supplierName) params.set("counterpartName", supplierName);
      if (status !== "all") params.set("status", status);
      if (startDate) params.set("startDate", startDate);
      if (endDate) params.set("endDate", endDate);

      const res = await fetch(`/api/pt-pks/keuangan/upah-bongkar?${params.toString()}`);
      if (!res.ok) {
        throw new Error("Gagal memuat upah bongkar");
      }

      const data = (await res.json()) as UpahBongkarResponse;
      setResponse(data);
      setSelectedItem((current) => {
        if (!current) return current;
        return data.items.find((item) => item.id === current.id) ?? null;
      });
      setSelectedIds((current) => current.filter((id) => data.items.some((item) => item.sourceId === id && item.status !== "PAID")));
    } catch (error) {
      console.error("Error fetching unloading wages:", error);
      toast.error("Gagal memuat upah bongkar");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  useEffect(() => {
    void fetchData();
  }, [vendorName, supplierName, status, startDate, endDate]);

  const openPaymentDialog = (item: UpahBongkarItem) => {
  ...
  const handlePayment = async () => {
    ...
    if (amount > selectedItem.sisaHutang) {
      toast.error("Jumlah bayar melebihi sisa upah bongkar");
      return;
    }

    try {
      setProcessingPayment(true);
      const res = await fetch("/api/pt-pks/keuangan/upah-bongkar/bayar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          penerimaanId: selectedItem.sourceId,
          jumlahBayar: amount,
      ...
      toast.success("Pembayaran upah bongkar berhasil dicatat");
  ...
  const handleMarkPaid = async (item: UpahBongkarItem) => {
    if (!confirm(`Tandai ${item.nomorReferensi} sebagai lunas?`)) {
      return;
    }

    try {
      const res = await fetch("/api/pt-pks/keuangan/upah-bongkar/lunas", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ penerimaanId: item.sourceId }),
      });
      ...
      toast.success("Upah bongkar berhasil ditandai lunas");
      await fetchData(true);
    } catch (error: any) {
      console.error("Error settling unloading wage:", error);
  ...
  const handleBatchMarkPaid = async () => {
    ...
    if (!confirm(`Lunasi ${selectedIds.length} data upah bongkar terpilih?`)) {
      return;
    }

    try {
      const res = await fetch("/api/pt-pks/keuangan/upah-bongkar/batch-lunas", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      toast.success(`${selectedIds.length} upah bongkar berhasil dilunasi`);
      setSelectedIds([]);
      await fetchData(true);
    } catch (error: any) {
      console.error("Error batch settling unloading wages:", error);
      toast.error(error.message || "Gagal melunasi data terpilih");
    }
  };

  const handleCancelPayment = async (paymentId: string) => {
    if (!confirm("Batalkan pembayaran ini? Saldo tagihan akan dihitung ulang.")) {
      return;
    }

    try {
      const res = await fetch(`/api/pt-pks/keuangan/upah-bongkar/pembayaran/${paymentId}`, {
        method: "DELETE",
      });
  ...
  const vendorOptions = useMemo(() => {
    const uniqueNames = Array.from(
      new Set(items.map((item) => item.vendorBongkarNama).filter(Boolean)),
    ).sort((a, b) =>
  ...
  const supplierOptions = useMemo(() => {
    const uniqueNames = Array.from(
      new Set(items.map((item) => item.counterpart).filter(Boolean)),
    ).sort((a, b) => a.localeCompare(b, "id-ID"));

    return [
      { value: "__all__", label: "Semua Supplier" },
      ...uniqueNames.map((name) => ({
        value: name,
        label: name,
      })),
  ...
  const selectableIds = items.filter((item) => item.status !== "PAID").map((item) => item.sourceId);
  ...
  const handleExportExcel = () => {
    ...
    const columns: ExportColumn[] = [
      ...
      { header: "Vendor Bongkar", key: "vendorBongkarNama", width: 25 },
      { header: "Tipe Vendor", key: "vendorBongkarTipe", width: 18 },
      { header: "Supplier", key: "counterpart", width: 25 },
      { header: "Netto (kg)", key: "beratDasar", width: 15 },
      { header: "Upah/Kg (Rp)", key: "hargaPerKg", width: 15 },
      { header: "Total Upah (Rp)", key: "totalNilai", width: 18 },
      { header: "Total Dibayar (Rp)", key: "totalDibayar", width: 18 },
      { header: "Sisa Tagihan (Rp)", key: "sisaHutang", width: 18 },
    ...
    const dataToExport = items.map((item) => {
      ...
      return {
        ...
        vendorBongkarNama: item.vendorBongkarNama,
        vendorBongkarTipe: item.vendorBongkarTipe || "-",
        counterpart: item.counterpart,
        beratDasar: item.beratDasar,
    ...
    exportToExcel(dataToExport, columns, "Upah_Bongkar_TBS");
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold">Upah Bongkar</h1>
          <p className="text-muted-foreground">
            Pembayaran upah bongkar untuk vendor bongkar dari transaksi penerimaan TBS yang sudah selesai.
          </p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-4">
          <Card>
            <CardHeader className="pb-2">
              <CardDescription>Total Upah Bongkar</CardDescription>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Filter data upah bongkar berdasarkan status, kata kunci, dan tanggal penerimaan.</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4 md:flex-row">
          <div className="w-full md:max-w-[220px]">
            <SearchableSelect
              ...
              onValueChange={(value) => setVendorName(value === "__all__" ? "" : value)}
              placeholder="Pilih nama vendor"
              searchPlaceholder="Cari nama vendor..."
              emptyMessage="Nama vendor tidak ditemukan"
            />
          </div>
          <div className="w-full md:max-w-[220px]">
            <SearchableSelect
              options={supplierOptions}
              value={supplierName || "__all__"}
              onValueChange={(value) => setSupplierName(value === "__all__" ? "" : value)}
      ...
      <Card>
        <CardHeader>
          <CardTitle>Daftar Upah Bongkar</CardTitle>
          <CardDescription>
            Nilai pembayaran memakai data vendor bongkar dan total upah bongkar dari penerimaan TBS.
          </CardDescription>
        </CardHeader>
        <CardContent>
                ...
                {loading ? (
                  ...
                  paginatedItems.map((item) => {
                    ...
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        <div>{item.vendorBongkarNama}</div>
                        <div className="text-xs text-muted-foreground">{item.vendorBongkarTipe || "-"}</div>
                      </TableCell>
                      <TableCell>{item.counterpart}</TableCell>
                      <TableCell className="text-right">{item.beratDasar.toLocaleString("id-ID")} kg</TableCell>
                      <TableCell className="text-right">{item.hargaPerKg.toLocaleString("id-ID")}</TableCell>
      ...
      <Dialog open={showPaymentDialog} onOpenChange={setShowPaymentDialog}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Catat Pembayaran Upah Bongkar</DialogTitle>
            <DialogDescription>
              {selectedItem ? `${selectedItem.nomorReferensi} - ${selectedItem.vendorBongkarNama}` : "Pilih data upah bongkar"}
            </DialogDescription>
          </DialogHeader>
          {selectedItem && (

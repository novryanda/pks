...
export function PembayaranPRList() {
  const [response, setResponse] = useState<PRResponse | null>(null);
  ...
  const fetchData = async (showRefreshState = false) => {
    try {
      ...
      const res = await fetch(`/api/pt-pks/keuangan/pembayaran-pr?${params.toString()}`);
      if (!res.ok) {
        throw new Error("Gagal memuat pembayaran PR");
      }

      const data = (await res.json()) as PRResponse;
      setResponse(data);
    } catch (error) {
      console.error("Error fetching pembayaran PR:", error);
      toast.error("Gagal memuat pembayaran PR");
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  const openPaymentDialog = (item: PRItem) => {
  ...
  const handlePayment = async () => {
    ...
    if (amount > selectedItem.sisaHutang) {
      toast.error("Jumlah pembayaran melebihi sisa PR");
      return;
    }

    try {
      setProcessingPayment(true);
      const res = await fetch(`/api/pt-pks/keuangan/pembayaran-pr/${selectedItem.id}/bayar`, {
        method: "POST",
      ...
      if (!res.ok) {
        ...
        throw new Error(error.error || "Gagal menyimpan pembayaran PR");
      }

      toast.success("Pembayaran PR berhasil dicatat");
      setShowPaymentDialog(false);
      await fetchData(true);
    } catch (error: any) {
      console.error("Error paying PR:", error);
      toast.error(error.message || "Gagal menyimpan pembayaran PR");
    } finally {
  ...
  const handleExportExcel = () => {
    ...
    const columns: ExportColumn[] = [
      ...
      { header: "No. PR", key: "nomorReferensi", width: 20 },
    ...
    exportToExcel(dataToExport, columns, "Pembayaran_PR");
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold">Pembayaran PR Langsung</h1>
          <p className="text-muted-foreground">Menampilkan hanya PR pembelian langsung dengan status bayar dan filter periode.</p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Filter berdasarkan status pembayaran dan periode tanggal PR.</CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-4 md:flex-row">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder="Cari nomor PR, vendor, atau divisi"
              className="pl-9"
            />
          </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>Daftar PR Langsung</CardTitle>
          <CardDescription>Nilai PR dihitung dari item permintaan dan pembayaran yang sudah tercatat.</CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <Dialog open={showPaymentDialog} onOpenChange={setShowPaymentDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedItem ? `${selectedItem.nomorReferensi} - ${selectedItem.vendorNama}` : "Pilih PR"}
            </DialogDescription>
          </DialogHeader>
          {selectedItem && (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="pr-payment-date">Tanggal Pembayaran</Label>
                <Input
                  id="pr-payment-date"
                  type="date"
                  value={paymentDate}
              ...
              <div className="space-y-2">
                <Label htmlFor="pr-payment-amount">Jumlah Bayar</Label>
                <Input
                  id="pr-payment-amount"
                  type="number"
                  value={paymentAmount}
              ...
              <div className="space-y-2">
                <Label htmlFor="pr-payment-method">Metode Pembayaran</Label>
                <Select value={paymentMethod} onValueChange={setPaymentMethod}>
                  <SelectTrigger id="pr-payment-method">
              ...
              <div className="space-y-2">
                <Label htmlFor="pr-payment-reference">No. Referensi</Label>
                <Input
                  id="pr-payment-reference"
              ...
              <div className="space-y-2">
                <Label htmlFor="pr-payment-note">Keterangan</Label>
                <Textarea
                  id="pr-payment-note"
                  value={paymentNote}

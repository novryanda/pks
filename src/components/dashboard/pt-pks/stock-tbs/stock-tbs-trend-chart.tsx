...
import { formatDateInJakarta, getJakartaDateKey, parseJakartaDateBoundary } from "@/lib/date-time";
...
export function StockTBSTrendChart() {
    ...
    const weekAgo = getJakartaDateKey(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))!;
    ...
    const fetchMaterials = async () => {
        try {
            ...
            if (res.ok) {
                ...
                const tbsMaterials = data.filter(
                    (m: Material) =>
    ...
    const fetchTrendData = async (
        ...
        try {
            const url = `/api/pt-pks/stock-tbs/daily-trend?materialId=${materialId}&startDate=${start}&endDate=${end}`;
    ...
    const resetFilters = () => {
        ...
        const weekAgoDate = getJakartaDateKey(new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))!;
        setFilterMode("range");
    ...
    const formatDate = (dateStr: string) => {
        const date = parseJakartaDateBoundary(dateStr);
        return date
                ...
                <CardContent className="pt-6">
                    <div className="text-center py-8">
                        ...
                        <p className="text-muted-foreground">
                            Belum ada material TBS terdaftar.
                        </p>
                    </div>
                </CardContent>
            </Card>
        ...
        <div className="space-y-6">
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Visualisasi pergerakan TBS (masuk, terpakai produksi, sisa stok) per
                        hari
                    </CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <Card>
                <CardHeader>
                    <CardTitle>
                        Grafik Pergerakan {selectedMaterialData?.name || "TBS"}
                    </CardTitle>
                    <CardDescription>
                        Data harian dari{" "}
                        {formatDateInJakarta(effectiveDates.startDate)} s/d{" "}
                        {formatDateInJakarta(effectiveDates.endDate)}
                    </CardDescription>
                </CardHeader>
                <CardContent>
                    {loading ? (
                    ...
                    <Card className="border-orange-200 bg-orange-50">
                        <CardContent className="pt-6">
                            ...
                            <div className="text-2xl font-bold text-orange-900">
                                {trendData
                                    .reduce((sum, d) => sum + d.tbsTerpakai, 0)
                                    .toLocaleString("id-ID", { maximumFractionDigits: 2 })}{" "}
                                <span className="text-sm font-normal">
                                    {selectedMaterialData?.satuan.symbol || "kg"}
                                </span>
